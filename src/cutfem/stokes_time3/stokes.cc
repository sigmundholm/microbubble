...
namespace examples::cut::StokesEquation2 {
    ...
    assemble_matrix() {
        ...
        for (const auto &cell : this->dof_handlers.front()->active_cell_iterators()) {
            ...
            // Compute and add the pressure stabilisation.
            pressure_stab.compute_stabilization(cell);
            pressure_stab.add_stabilization_to_matrix(-gamma_A, // * this->tau / nu, TODO fixit
                                                      this->stiffness_matrix);
        }
    }
    ...
    template<int dim>
    void
    StokesEqn2<dim>::assemble_matrix_local_over_cell(
        ...
        for (unsigned int q = 0; q < fe_values.n_quadrature_points; ++q) {
            ...
            for (const unsigned int i : fe_values.dof_indices()) {
                for (const unsigned int j : fe_values.dof_indices()) {
                    local_matrix(i, j) +=
                            (this->bdf_coeffs[0] * phi_u[j] * phi_u[i]  // (u, v)
                             +
    ...
    template<int dim>
    void
    StokesEqn2<dim>::assemble_matrix_local_over_surface(
    ...
    assemble_rhs(int time_step, bool moving_domain) {
        ...
        for (const auto &cell : this->dof_handlers.front()->active_cell_iterators()) {
            ...
            for (const auto &face : cell->face_iterators()) {
                ...
                if (face->boundary_id() == do_nothing_id) {
                    std::cout << "ADFLØJKDFLJK" << std::endl;
                }
            }
    ...
    template<int dim>
    void
    StokesEqn2<dim>::assemble_rhs_local_over_surface(
            const FEValuesBase <dim> &fe_values,
            const std::vector <types::global_dof_index> &loc2glb) {
        // Matrix and vector for the contribution of each cell
        const unsigned int dofs_per_cell = fe_values.get_fe().dofs_per_cell;
