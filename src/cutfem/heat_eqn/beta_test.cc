...
void solve_for_element_order(int element_order, int max_refinement, 
    ...
    std::ofstream meta("e-meta-d" + std::to_string(dim)
                       + "o" + std::to_string(element_order)
                       + "beta=" + std::to_string(beta0) + ".txt");
    meta << "Heat equation convergence test" << std::endl
         << "==============================" << std::endl
...
void run_convergence_test(std::vector<int> orders, int max_refinement,
                          std::vector<double> betas, bool write_output) {
    for (int order : orders) {
        for (double beta0 : betas) {
            std::cout << "dim=" << dim << ", element_order=" << order
                    << std::endl;
            solve_for_element_order<dim>(order, max_refinement, beta0, 
                                         write_output);
        }
    }
}
...
int main() {
    run_convergence_test<2>({1}, 7, {0.5, 5, 10, 15, 20, 30, 50}, false);
}