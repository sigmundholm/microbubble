...
def convergence_plot_report():
    ...
    conv_plots2(paths, plot_for, element_orders, expected_degrees=[[2, 2], [1, 2], [2, 2], [1, 2]], domain_length=2.2,
                save_figs=True, font_size=12, label_size="large", skip=0,
                guess_degree=False)
...
def time_error_plot():
    for d in [1, 2]:
        paths = [os.path.join(base, f"build/src/cutfem/stokes_time2", folder, f"errors-time-d2o{d}r{r}.csv") for r in
                 range(2, 8)]
        time_error_plots(paths, end_time=1.1, data_indices=[2, 3, 4, 5, 6, 7], font_size=12, label_size="large",
                         title=f"Stokes Equations time error, element order ({d}", save_fig=True, identifier=d)
